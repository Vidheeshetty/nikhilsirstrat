...
class ReportGenerator:
    ...
    def write_summary_with_trades(self, log_file: str, summary_data: Dict[str, Any], 
        ...
        with open(log_file, "a") as f:
            ...
            f.write(f"Starting Balance: {summary_data.get('starting_balance',0.0):,.2f} {summary_data.get('base_currency','INR')}\n")
            f.write(f"Ending Balance: {summary_data.get('ending_balance',0.0):,.2f} {summary_data.get('base_currency','INR')}\n")
            f.write(f"Free Balance: {summary_data.get('balance_free',0.0):,.2f} {summary_data.get('base_currency','INR')}\n")
            f.write(f"Locked Balance: {summary_data.get('balance_locked',0.0):,.2f} {summary_data.get('base_currency','INR')}\n")
            f.write("\n")
    ...
    def write_summary_only(self, log_file: str, summary_data: Dict[str, Any]) -> None:
        ...
        with open(log_file, "a") as f:
            ...
            f.write(f"Total PnL: {self._safe_float_format(summary_data.get('pnl_total'), '{:,.2f}', 'N/A')} {summary_data.get('currency', 'INR')}\n")
            f.write(f"Total PnL %: {self._safe_float_format(summary_data.get('pnl_pct_total'), '{:.2f}%', 'N/A')}\n")
            f.write(f"Total Investment: {self._safe_float_format(summary_data.get('total_investment'), '{:,.2f}', 'N/A')} {summary_data.get('currency', 'INR')}\n")
            ...
            f.write(f"Realized PnL: {self._safe_float_format(summary_data.get('realized_pnl'), '{:,.2f}', 'N/A')}\n")
            f.write(f"Unrealized PnL: {self._safe_float_format(summary_data.get('unrealized_pnl'), '{:,.2f}', 'N/A')}\n")
            f.write(f"Sharpe Ratio: {self._safe_float_format(summary_data.get('sharpe_ratio'), '{:.2f}', 'N/A')}\n")
            f.write(f"Starting Balance: {self._safe_float_format(summary_data.get('starting_balance'), '{:,.2f}', 'N/A')} {summary_data.get('base_currency','INR')}\n")
            f.write(f"Ending Balance: {self._safe_float_format(summary_data.get('ending_balance'), '{:,.2f}', 'N/A')} {summary_data.get('base_currency','INR')}\n")
            f.write(f"Free Balance: {self._safe_float_format(summary_data.get('balance_free'), '{:,.2f}', 'N/A')} {summary_data.get('base_currency','INR')}\n")
            f.write(f"Locked Balance: {self._safe_float_format(summary_data.get('balance_locked'), '{:,.2f}', 'N/A')} {summary_data.get('base_currency','INR')}\n")
            f.write("\n")
    
    def write_single_instrument_batch_log(self, batch_log, result: Dict[str, Any]) -> None:
    ...
    def create_consolidated_summary(self, all_results: List[Dict[str, Any]], 
                                  output_file: str) -> str:
        ...
        total_investment = sum(self._parse_currency_value(r.get("total_investment", "0.0 INR")) for r in all_results)
        total_pnl = sum(self._parse_currency_value(r.get("pnl_total", "0.0")) for r in all_results)
        ...
        total_unrealized_pnl = sum(float(r.get("unrealized_pnl", 0.0)) for r in all_results)
        # For Sharpe Ratio, calculate average, handling potential non-numeric values
        sharpe_ratios = [float(r.get("sharpe_ratio", 0.0)) for r in all_results if isinstance(r.get("sharpe_ratio"), (int, float))]
        total_sharpe_ratio = sum(sharpe_ratios) / len(sharpe_ratios) if sharpe_ratios else 0.0

        total_start_balance = sum(float(r.get("starting_balance", 0.0)) for r in all_results)
        total_end_balance = sum(float(r.get("ending_balance", 0.0)) for r in all_results)
        ...
        with open(output_file, "w") as f:
            ...
            f.write("-" * 190 + "\n")
            f.write(f"{'Instrument':<40} | {'Orders':>8} | {'Positions':>10} | {'Trades':>8} | {'Investment':>15} | {'PnL':>12} | {'PnL%':>7} | {'Realized':>14} | {'Unrealized':>15} | {'Sharpe':>8} | {'Start Bal':>10} | {'End Bal':>10}\n")
            f.write("-" * 190 + "\n")
    ...
    def _parse_currency_value(self, value_str: str) -> float:
        ...
        try:
            ...
            clean_value = value_str.replace("INR", "").replace("₹", "").replace(",", "").strip()
            return float(clean_value)
        except (ValueError, TypeError):
            return 0.0 # Return 0.0 if parsing fails
    
    def create_tabular_summary(self, all_results: List[Dict[str, Any]], output_file: str):
        ...
        overall_pnl_pct = (total_pnl / total_investment * 100) if total_investment > 0 else 0.0
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"Net Account Change: {ending_balance - starting_balance:,.2f} {currency}\n\n")
            ...
            # Write header
            headers = [
                'Instrument ID', 'Orders', 'Positions', 'Trades', 
                'Investment', 'PnL', 'PnL%', 'Realized', 'Unrealized', 'Sharpe',
                'Start Bal', 'End Bal', 'Free Bal', 'Locked Bal'
            ]
            f.write(f"{'Instrument ID':<25} {'Orders':<6} {'Pos':<6} {'Trades':<6} "
            ...
            if pnl_values:
                ...
                f.write(f"Average PnL: {sum(pnl_values) / len(pnl_values):,.2f} {currency}\n")
    ...
    def write_batch_results_summary(self, log_file: str, all_results: List[Dict[str, Any]]):
        """Write a detailed BACKTEST RESULTS SUMMARY for batch runs, aggregating totals and averages."""
        total_orders = sum(r.get('total_orders', 0) for r in all_results)
        total_positions = sum(r.get('total_positions', 0) for r in all_results)
        total_trades = sum(r.get('total_trades', 0) for r in all_results)
        total_investment = sum(float(str(r.get('total_investment', '0.0')).split()[0]) for r in all_results)
        total_pnl = sum(float(r.get('pnl_total', '0.00').replace(',', '')) for r in all_results)
        total_realized_pnl = sum(float(r.get('realized_pnl', '0.00').replace(',', '')) for r in all_results)
        total_unrealized_pnl = sum(float(r.get('unrealized_pnl', '0.00').replace(',', '')) for r in all_results)
        avg_sharpe = (
            sum(float(r.get('sharpe_ratio', '0.0000')) for r in all_results if r.get('sharpe_ratio')) / max(1, len([r for r in all_results if r.get('sharpe_ratio')]))
        )
        total_starting_balance = sum(float(r.get('starting_balance', 0.0)) for r in all_results)
        total_ending_balance = sum(float(r.get('ending_balance', 0.0)) for r in all_results)
        total_free_balance = sum(float(r.get('balance_free', 0.0)) for r in all_results)
        total_locked_balance = sum(float(r.get('balance_locked', 0.0)) for r in all_results)
        currency = all_results[0].get('currency', 'INR') if all_results else 'INR'
        base_currency = all_results[0].get('base_currency', 'INR') if all_results else 'INR'
        ...
        with open(log_file, 'a') as f:
            ...
            f.write(f"Starting Balance (sum): {total_starting_balance:,.2f} {base_currency}\n")
            f.write(f"Ending Balance (sum): {total_ending_balance:,.2f} {base_currency}\n")
            f.write(f"Free Balance (sum): {total_free_balance:,.2f} {base_currency}\n")
            f.write(f"Locked Balance (sum): {total_locked_balance:,.2f} {base_currency}\n")
            f.write("\n" + "=" * 80 + "\n")
            f.write("END OF REPORT\n")
            f.write("=" * 80 + "\n") 