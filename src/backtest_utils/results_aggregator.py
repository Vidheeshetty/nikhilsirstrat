...
class ResultsAggregator:
    ...
    def aggregate_results(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for result in results:
            if result and "result" in result:
                try:
                    ...
                    # Aggregate metrics
                    aggregated["total_return"] += getattr(backtest_result, 'total_return', 0.0)
                    aggregated["total_trades"] += getattr(backtest_result, 'total_trades', 0)
        ...
        if aggregated["successful_runs"] > 0:
            aggregated["avg_return"] = aggregated["total_return"] / aggregated["successful_runs"]
            aggregated["avg_trades"] = aggregated["total_trades"] / aggregated["successful_runs"]
            
            # Calculate average Sharpe ratio
            sharpe_sum = sum(getattr(r, 'sharpe_ratio', 0.0) for r in successful_results)
    ...
    def save_aggregated_results(
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"Successful Runs: {aggregated_results.get('successful_runs', 0)}\n")
            f.write(f"Failed Runs: {aggregated_results.get('failed_runs', 0)}\n")
            f.write(f"Total Return: {aggregated_results.get('total_return', 0.0):.4f}\n")
            f.write(f"Average Return: {aggregated_results.get('avg_return', 0.0):.4f}\n")
            f.write(f"Total Trades: {aggregated_results.get('total_trades', 0)}\n")
            f.write(f"Average Trades: {aggregated_results.get('avg_trades', 0.0):.2f}\n")
            f.write(f"Win Rate: {aggregated_results.get('win_rate', 0.0):.4f}\n")
            f.write(f"Sharpe Ratio: {aggregated_results.get('sharpe_ratio', 0.0):.4f}\n")
            f.write(f"Max Drawdown: {aggregated_results.get('max_drawdown', 0.0):.4f}\n")
