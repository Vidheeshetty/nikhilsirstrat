...
class ResultsProcessor:
    ...
    def extract_detailed_data(self, engine, result, log_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            equity_curve = []
            timestamps = []
            running_balance = account_data["starting_balance"] if account_data else 1_000_000.0
            for pos in positions:
                ts_closed = pos.get("ts_closed")
                realized_pnl = pos.get("realized_pnl", "0.0")
                try:
                    realized_pnl_val = float(str(realized_pnl).replace("INR", "").replace("₹", "").replace(",", "").strip())
                except Exception:
            ...
            if not equity_curve:
                equity_curve = [account_data["starting_balance"], account_data["ending_balance"]]
    ...
    def _extract_account_data(self, engine, result=None) -> dict:
        ...
        if result is not None and hasattr(result, 'account_balances') and result.account_balances is not None:
            try:
                ...
                if hasattr(result, 'final_balances'):
                    ...
                    if hasattr(final_balances, 'values') and len(final_balances.values) > 0:
                ...
                print(f"[WARNING] Failed to extract balances from BacktestResult.account_balances: {e}")
        accounts = engine.cache.accounts() if hasattr(engine.cache, 'accounts') else []
        if accounts and len(accounts) > 0:
            account = accounts[0]
            ending_balance = float(account.balance_total() if callable(getattr(account, 'balance_total', None)) else getattr(account, 'balance_total', 0.0))
            return {
                "account_id": getattr(account, 'account_id', 'N/A'),
                "base_currency": getattr(account, 'base_currency', 'N/A'),
                "starting_balance": STARTING_BALANCE,
                "ending_balance": ending_balance,
                "balance_free": float(account.balance_free() if callable(getattr(account, 'balance_free', None)) else getattr(account, 'balance_free', 0.0)),
                "balance_locked": float(account.balance_locked() if callable(getattr(account, 'balance_locked', None)) else getattr(account, 'balance_locked', 0.0)),
            }
        else:
            print("[WARNING] No account found in engine.cache.accounts(). Returning zeros.")
            return {
                "account_id": "N/A",
                "base_currency": "N/A",
                "starting_balance": STARTING_BALANCE,
    ...
    def extract_returns_series(self, result, detailed_data) -> pd.Series:
        ...
        if equity_curve is not None and isinstance(equity_curve, (pd.Series, np.ndarray, list)):
    ...
    def extract_pnl_data(self, result, detailed_data=None) -> Dict[str, float]:
        ...
        if realized_pnl in [None, 'N/A'] or abs(realized_pnl) < 1e-8:
            ...
            if detailed_data is not None:
                ...
                for pos in positions:
                    ...
                    if side_val == 'FLAT':
                        val = str(pos.get('realized_pnl', '0.0')).replace('INR', '').replace('₹', '').replace(',', '').strip()
                        try:
        ...
        if (unrealized_pnl in [None, 'N/A'] or abs(unrealized_pnl) < 1e-8) and detailed_data is not None:
            positions = detailed_data.get('positions', [])
            # PATCH: Use last available price for open positions if possible
            last_prices = detailed_data.get('last_prices', {})  # Should be a dict: instrument_id -> last_price
            open_positions = []
            unrealized_sum = 0.0
            unrealized_values = []
        ...
        if sharpe_ratio == 'N/A':
            ...
            if not returns.empty:
                try:
                    ...
                    sharpe_ratio = float(sharpe) if sharpe is not None and not np.isnan(sharpe) else 'N/A'
                except Exception as e:
    ...
    def create_summary_data(self, result, detailed_data: Dict[str, Any], instrument_id: str, batch_mode: bool = False) -> Dict[str, Any]:
