...
ticks_parquet = catalog.query(data_cls=QuoteTick, identifiers=[instrument_id.value])
...
if len(df_csv_filtered) > 0:
    ...
    csv_display = df_csv_filtered[
        ["timestamp", "bid", "ask", "totalBuyQuantity", "totalSellQuantity"]
    ].head(10)
...
if len(df_csv_filtered) > 0 and len(df_parquet) > 0:
    print("\nCSV Statistics:")
    print(
        f"  Bid range: {df_csv_filtered['bid'].min():.2f} to {df_csv_filtered['bid'].max():.2f}"
    )
    print(
        f"  Ask range: {df_csv_filtered['ask'].min():.2f} to {df_csv_filtered['ask'].max():.2f}"
    )
    ...
    print("\nParquet Statistics:")
    print(
        f"  Bid range: {df_parquet['bid'].min():.2f} to {df_parquet['bid'].max():.2f}"
    )
    print(
        f"  Ask range: {df_parquet['ask'].min():.2f} to {df_parquet['ask'].max():.2f}"
    )
    ...
    if (
        abs(csv_bid_range - parquet_bid_range) < 1.0
        and abs(csv_ask_range - parquet_ask_range) < 1.0
    ):
...
if len(df_csv_filtered) != len(df_parquet):
    ...
    if len(df_csv_filtered) == 1 and len(df_parquet) > 1:
        ...
        print(
            "This is expected if the CSV is end-of-day data and Parquet contains intraday data."
        )
        ...
        print(
            df_csv_filtered[
                ["timestamp", "symbol", "bid", "ask", "strike", "expiry"]
        ...
        print(
            df_parquet[["timestamp", "bid", "ask", "bid_size", "ask_size"]]
            .head(5)
            .to_string(index=False)
        )
