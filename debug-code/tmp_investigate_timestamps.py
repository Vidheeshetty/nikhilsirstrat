...
if len(ticks) > 0:
    ...
    for i, tick in enumerate(ticks[:20]):
        ...
        print(f"  {i+1:2d}. {dt}: bid={float(tick.bid_price):.2f}, ask={float(tick.ask_price):.2f}")
    
    print(f"\nLast 20 timestamps:")
    for i, tick in enumerate(ticks[-20:]):
        dt = datetime.fromtimestamp(tick.ts_init / 1e9)
        print(f"  {len(ticks)-19+i:2d}. {dt}: bid={float(tick.bid_price):.2f}, ask={float(tick.ask_price):.2f}")
