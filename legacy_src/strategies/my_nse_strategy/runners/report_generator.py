...
class ReportGenerator:
    ...
    def create_consolidated_summary(self, all_results: List[Dict[str, Any]], 
        ...
        with open(output_file, "w") as f:
            ...
            f.write("-" * 120 + "\n")
            f.write(f"{'Instrument':<40} | {'Orders':>8} | {'Positions':>10} | {'Trades':>8} | {'Investment':>15} | {'PnL':>12} | {'PnL%':>8} | {'Realized':>10} | {'Unrealized':>11} | {'Sharpe':>8} | {'Start Bal':>10} | {'End Bal':>10}\n")
            f.write("-" * 180 + "\n")
            for result in all_results:
                ...
                f.write(
                    ...
                    f"{result['total_investment']:>15} | {result['pnl_total']:>12} | {result['pnl_pct_total']:>8} | "
                    f"{result.get('realized_pnl','0.00'):>10} | {result.get('unrealized_pnl','0.00'):>11} | {result.get('sharpe_ratio','0.0000'):>8} | "
                    f"{result.get('starting_balance',0.0):>10,.0f} | {result.get('ending_balance',0.0):>10,.0f}\n"
                )
            f.write("=" * 180 + "\n")
    ...
    def _parse_currency_value(self, value_str: str) -> float:
        """Extract float value from currency string like '123.45 INR'."""
