...
class DataManager:
    ...
    def _cleanup_resource_forks(self) -> None:
        ...
        if removed:
            print(
                f"DataManager: removed {removed} macOS resource-fork files from catalog"
            )
    ...
    def get_instrument(self, instrument_id: str):
        ...
        insts = self.catalog.instruments(
            instrument_ids=[instrument_id], as_nautilus=True
        )
        if insts:
            return insts[0]
        ...
        if ".FUT." in instrument_id:
            ...
            if base_symbol == base_symbol_with_expiry:
                cands = [
                    ins
                    for ins in self.catalog.instruments(as_nautilus=True)
                    if ins.raw_symbol.value.startswith(base_symbol)
                    and ins.id.value.endswith(f".FUT.{venue}")
                ]
                if cands:
            ...
            insts_c = self.catalog.instruments(
                instrument_ids=[continuous_id], as_nautilus=True
            )
            if insts_c:
                return insts_c[0]
    ...
    def _resolve_instrument_ids(self, instrument_id: str) -> List[str]:
        ...
        if ".FUT." in instrument_id and not any(
            ch.isdigit() for ch in instrument_id.split(".")[0]
        ):
            base, _, venue = instrument_id.partition(".FUT.")
            return [
                ins.id.value
                for ins in self.catalog.instruments()
                if ins.raw_symbol.value.startswith(base)
                and ins.id.value.endswith(f".FUT.{venue}")
            ]
    ...
    def get_bars(self, instrument_id: str, start, end) -> List[Bar]:
        ...
        for iid in ids:
            ...
            try:
                chunk = self.catalog.bars(
                    bar_types=[bar_type], start=start_ns, end=end_ns, as_nautilus=True
                )
                bars.extend(chunk)
    ...
    def get_quote_ticks(
        self, instrument_id: str, start: str, end: str
    ) -> List[QuoteTick]:
        # Try real ticks first
        try:
            qts = self.catalog.quote_ticks(
                instrument_ids=[instrument_id], start=start, end=end, as_nautilus=True
            )
            if qts:
                return qts
        except Exception:
        ...
        for bar in bars:
            inst_id = (
                ...
                if hasattr(bar.bar_type, "instrument_id")
