...
def main():
    ...
    catalog_path = "catalog-data/source=NSER/instrument=FUT/venue=NSE/symbol=NIFTY/timeframe=DAILY"
    ...
    if bars:
        ...
        print(f"  OHLC: {float(first_bar.open):.2f}, {float(first_bar.high):.2f}, {float(first_bar.low):.2f}, {float(first_bar.close):.2f}")
        
        print(f"\nLast trading day:")
        print(f"  Date: {last_bar.ts_event}")
        print(f"  OHLC: {float(last_bar.open):.2f}, {float(last_bar.high):.2f}, {float(last_bar.low):.2f}, {float(last_bar.close):.2f}")
        ...
        print("    ohlc = (float(bar.open), float(bar.high), float(bar.low), float(bar.close))")
