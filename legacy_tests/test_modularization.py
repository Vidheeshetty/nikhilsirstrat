...
def _run_backtest_directly(instrument_id: str, verbose: bool = False, max_workers: int = 1) -> str:
    ...
    if instrument_id == "all":
        results = runner.run_all_instruments_backtest(MyNSEStrategy, verbose=verbose, max_workers=max_workers)
    elif instrument_id == "all-top10":
        all_ids = runner.data_manager.get_all_instrument_ids()
        results = runner.run_batch_backtest(all_ids[:10], MyNSEStrategy, verbose=verbose, max_workers=max_workers)
    else:
        results = runner.run_single_backtest(instrument_id, MyNSEStrategy, verbose=verbose)

    # Find the newest log file
    log_files = [os.path.join(LOG_DIR, f) for f in os.listdir(LOG_DIR) if f.endswith(".log") or f.endswith(".txt")]
    if not log_files:
        raise FileNotFoundError(f"No log file found in {LOG_DIR} for instrument {instrument_id}")
...
def test_single_instrument_report_format():
    ...
    if not instrument_id:
        pytest.skip("Could not find any instrument with valid data for the test period.")
...
def test_ten_instruments_batch_report_format():
    ...
    for match_tuple in all_matches:
        ...
        if realized_pnl_str != "N/A":
            try:
                float(realized_pnl_str.replace(",", "")) # Remove commas before converting
            except ValueError:
                assert False, f"Realized PnL value '{realized_pnl_str}' is not a valid float or N/A"

        # Validate Unrealized PnL (group 5, index 4)
        if unrealized_pnl_str != "N/A":
            try:
                float(unrealized_pnl_str.replace(",", "")) # Remove commas before converting
            except ValueError:
                assert False, f"Unrealized PnL value '{unrealized_pnl_str}' is not a valid float or N/A"
        ...
        if sharpe_ratio_str != "N/A":
            try:
                ...
                assert False, f"Sharpe Ratio value '{sharpe_ratio_str}' is not a valid float or N/A"
