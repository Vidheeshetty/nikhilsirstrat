...
"""
    ...
    mock.get_strategy_config = MagicMock(side_effect=lambda inst_id: MockMyNSEStrategyConfig(inst_id, **mock.config.copy()))
    mock.get_backtest_params = MagicMock(return_value={
        "start_time": "2025-06-18T09:15:00",
        "end_time": "2025-06-18T09:16:00",
        "base_currency": "INR",
    })
...
def mock_data_manager():
    ...
    mock.get_all_instrument_ids = MagicMock(return_value=["NIFTY.OPT.19Jun2025.24800.PUT.NSE"])
    ...
    mock_ticks = [
        Mock(ts_event=dt_to_unix_nanos(datetime(2025, 6, 18, 9, 15, 0, tzinfo=timezone.utc))),
        Mock(ts_event=dt_to_unix_nanos(datetime(2025, 6, 18, 9, 16, 0, tzinfo=timezone.utc))),
    ]
...
def mock_engine_manager():
    ...
    mock_nautilus_engine.get_result = MagicMock(return_value=Mock()) # Return a mock result
    ...
    mock_nautilus_engine.trader.generate_order_fills_report = MagicMock(return_value=pd.DataFrame())
    mock_nautilus_engine.trader.generate_positions_report = MagicMock(return_value=pd.DataFrame())
    ...
    mock.get_results = MagicMock(return_value=Mock()) # Should return a mock with relevant attributes
    ...
    mock_result.stats_pnls = {'INR': {'PnL (total)': 1000.0, 'PnL% (total)': 0.1, 'PnL (realized)': 900.0, 'PnL (unrealized)': 100.0, 'Sharpe Ratio (252 days)': 1.0}}
    ...
    mock_result.account_balances = pd.DataFrame([{'venue': 'NSE', 'currency': 'INR', 'total': 1001000.0}])
...
def mock_results_processor():
    ...
    mock.extract_detailed_data = MagicMock(return_value={
        "orders": [{"client_order_id": "ORD1"}],
        "positions": [{"id": "POS1"}],
        "trades": [{"trade_id": "TRD1", "side": "BUY", "price": "100.0", "quantity": "10"}],
        "account": {"starting_balance": 1000000.0, "ending_balance": 1001000.0, "base_currency": "INR"},
        "last_prices": {"NIFTY.OPT.19Jun2025.24800.PUT.NSE": 101.0}
    })
    mock.create_summary_data = MagicMock(return_value={
...
class TestBacktestOrchestratorIntegration:
    ...
    def test_run_single_backtest_success(self,
        ...
        mock_config_loader.config["catalog_path"] = str(tmp_path / "dummy_catalog") # Ensure catalog_path is set
        (tmp_path / "dummy_catalog" / "data" / "quote_tick" / "NIFTY.OPT.19Jun2025.24800.PUT.NSE").mkdir(parents=True, exist_ok=True)
        ...
        orchestrator = BacktestOrchestrator(
            ...
            catalog_path=mock_data_manager.catalog_path if hasattr(mock_data_manager, 'catalog_path') else str(tmp_path / "dummy_catalog"),
        ...
        orchestrator.config_manager.get_strategy_config.assert_called_once_with(instrument_id)
        orchestrator.config_manager.get_backtest_params.assert_called_once_with(start_time, end_time)
    ...
    def test_run_single_backtest_data_not_available(self,
                                                  mock_config_loader,
                                                  mock_data_manager,
                                                  mock_engine_manager,
                                                  mock_results_processor,
