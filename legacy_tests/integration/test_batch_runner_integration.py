...
class TestBatchRunnerIntegration:
    def test_run_batch_backtests(
        self, mock_backtest_orchestrator, mock_report_generator_for_batch, tmp_path
    ):
...
""")
        ...
        # Assertions
        assert mock_backtest_orchestrator.run_single_backtest.call_count == len(
            instrument_ids
        )
        ...
        args, kwargs = (
            mock_report_generator_for_batch.write_batch_results_summary.call_args
        )
        ...
        assert kwargs["summary_data"]["avg_pnl_total"] == pytest.approx(
            (1000.0 + 500.0) / 2
        )

    def test_run_all_instruments_backtests(
        self, mock_backtest_orchestrator, mock_report_generator_for_batch, tmp_path
    ):
...
""")
        ...
        mock_data_manager_for_all.get_all_instrument_ids = MagicMock(
            return_value=[
                "SUCCESS.INST.1",
                "FAILED.INST.2",
                "SUCCESS.INST.3",
        ...
        mock_data_manager_for_all.catalog_path = (
            "/mock/catalog/path"  # Ensure this is set
        )
        ...
        assert (
            mock_backtest_orchestrator.run_single_backtest.call_count == 4
        )  # Total calls
