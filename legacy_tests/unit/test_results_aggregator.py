...
def dummy_backtest_results_for_aggregator():
    ...
    class MockBacktestResult:
        def __init__(self, total_return, total_trades, sharpe_ratio, win_rate, max_drawdown, pnl_currency="INR"):
            self.total_return = total_return
            self.total_trades = total_trades
            self.sharpe_ratio = sharpe_ratio
            self.win_rate = win_rate
            self.max_drawdown = max_drawdown
            self.stats_pnls = {pnl_currency: {
                'PnL (total)': float(total_return) * 100000, # Simulate PnL in currency
...
def dummy_backtest_results_with_failures(dummy_backtest_results_for_aggregator):
    ...
    """
    failures = [
        {"error": "Data not found for instrument XXX", "instrument_id": "FAILED.INST.1"},
...
def dummy_detailed_data():
    ...
    return {
        ...
        "positions": [],
        "trades": [],
        "account": {"starting_balance": 100000.0, "ending_balance": 105000.0, "base_currency": "INR"},
...
class TestResultsAggregator:
    ...
    def test_aggregate_results_success(self, dummy_backtest_results_for_aggregator):
        ...
        aggregated_results = aggregator.aggregate_results(dummy_backtest_results_for_aggregator)

        assert aggregated_results["total_runs"] == 3
        assert aggregated_results["successful_runs"] == 3
        assert aggregated_results["failed_runs"] == 0
        assert aggregated_results["avg_return"] == pytest.approx((0.1 + 0.05 + 0.15) / 3)
        assert aggregated_results["avg_trades"] == pytest.approx((10 + 5 + 12) / 3)
        assert aggregated_results["sharpe_ratio"] == pytest.approx((1.5 + 1.0 + 1.8) / 3)
    ...
    def test_aggregate_results_with_failures(self, dummy_backtest_results_with_failures):
        aggregator = ResultsAggregator()
        aggregated_results = aggregator.aggregate_results(dummy_backtest_results_with_failures)
    ...
    def test_save_aggregated_results(self, dummy_backtest_results_for_aggregator, tmp_path):
        aggregator = ResultsAggregator()
    ...
    def test_extract_pnl_data_from_stats_pnls(self, dummy_backtest_results_for_aggregator, dummy_detailed_data):
        ...
        assert pnl_data["total_pnl"] == pytest.approx(mock_result.stats_pnls["INR"]['PnL (total)'])
        assert pnl_data["pnl_percentage"] == pytest.approx(mock_result.stats_pnls["INR"]['PnL% (total)'])
        assert pnl_data["realized_pnl"] == pytest.approx(mock_result.stats_pnls["INR"]['PnL (realized)'])
        assert pnl_data["unrealized_pnl"] == pytest.approx(mock_result.stats_pnls["INR"]['PnL (unrealized)'])
        assert pnl_data["sharpe_ratio"] == pytest.approx(mock_result.stats_pnls["INR"]['Sharpe Ratio (252 days)'])
    ...
    def test_create_summary_data_single_mode(self, dummy_backtest_results_for_aggregator, dummy_detailed_data):
        aggregator = ResultsAggregator()
        mock_result = dummy_backtest_results_for_aggregator[0]["result"]
        ...
        summary_data = aggregator.create_summary_data(mock_result, dummy_detailed_data, "TEST.INST.ID", batch_mode=False)

        assert summary_data["instrument_id"] == "TEST.INST.ID"
        assert summary_data["pnl_total"] == pytest.approx(mock_result.stats_pnls["INR"]['PnL (total)'])
        # For single mode, pnl_pct_total is calculated based on total_investment
        assert summary_data["pnl_pct_total"] == pytest.approx((mock_result.stats_pnls["INR"]['PnL (total)'] / 1000.0) * 100)
    ...
    def test_create_summary_data_batch_mode(self, dummy_backtest_results_for_aggregator, dummy_detailed_data):
        aggregator = ResultsAggregator()
        mock_result_dict = dummy_backtest_results_for_aggregator[0] # This is the dict containing "result": MockBacktestResult
        ...
        mock_result_dict['account'] = dummy_detailed_data['account'] # Ensure this is passed
        ...
        summary_data = aggregator.create_summary_data(mock_result_dict, dummy_detailed_data, "BATCH.TEST.ID", batch_mode=True)

        assert summary_data["instrument_id"] == "BATCH.TEST.ID"
        assert summary_data["total_orders"] == 10
        assert summary_data["total_positions"] == 5
        assert summary_data["total_trades"] == 1 # Based on dummy_detailed_data["trades"]
