...
def setup_csv_data_for_conversion(tmp_path_factory):
    ...
    """
    test_data_dir = tmp_path_factory.mktemp("test_csv_data") / "options" / "nse" / "nifty"
...
"""
    ...
    (temp_project_root / "data" / "options" / "nse" / "nifty").mkdir(parents=True, exist_ok=True)
    (temp_project_root / "catalog-data" / "my_nse_strategy" / "catalog").mkdir(parents=True, exist_ok=True)
    (temp_project_root / "catalog-data" / "my_nse_strategy" / "catalog-meta").mkdir(parents=True, exist_ok=True)
    
    # Copy the dummy CSV into the simulated data directory
    shutil.copytree(test_data_dir.parent.parent, temp_project_root / "data" / "options", dirs_exist_ok=True)
    
    # Return the path to the script in the temp environment and the temp project root
    script_path = Path(__file__).resolve().parent.parent.parent / "src" / "convert_csv_to_parquet_w_meta_fixed_v5.py"
    temp_script_path = temp_project_root / "src" / "convert_csv_to_parquet_w_meta_fixed_v5.py"
...
def test_csv_to_parquet_conversion_runs(setup_csv_data_for_conversion):
    ...
    try:
        ...
        catalog_meta_dir = temp_project_root / "catalog-data" / "my_nse_strategy" / "catalog-meta"
        ...
        assert any(catalog_dir.glob("**/*.parquet")), "Should contain parquet files in catalog"
        assert any(catalog_meta_dir.glob("**/*.parquet")), "Should contain parquet files in meta catalog"
