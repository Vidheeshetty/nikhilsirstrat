...
class PaperTradingEngine:
    ...
    async def _main_loop(self):
        ...
        market_start = time.fromisoformat(market_config.get('market_start_time', '09:15'))
        ...
        try:
            while self.running:
                ...
                update_freq = self.config.get('data_feed', {}).get('update_frequency', 1)
            ...
            for broker_name, balance in balances.items():
                if isinstance(balance, dict) and 'total_balance' in balance:
                    ...
                    if unrealized_pnl < -max_daily_loss:
                        logger.warning(f"Daily loss limit exceeded for {broker_name}: {unrealized_pnl}")
                        await self._emergency_stop(f"Daily loss limit exceeded: {unrealized_pnl}")
                        return
                    
                    # Check drawdown limit
                    max_drawdown = risk_config.get('max_drawdown_limit', 0.15)
                    initial_balance = getattr(self.broker_manager.get_broker(broker_name), '_initial_balance', 1000000)
                    current_drawdown = (initial_balance - total_balance) / initial_balance
                    
                    if current_drawdown > max_drawdown:
                        logger.warning(f"Drawdown limit exceeded for {broker_name}: {current_drawdown:.2%}")
                        await self._emergency_stop(f"Drawdown limit exceeded: {current_drawdown:.2%}")
                        return
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "strategies": {name: runner.get_status() for name, runner in self.strategy_runners.items()},
...
async def main():
    ...
    parser.add_argument(
        "--strategy",
        help="Specific strategy to run (optional)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
