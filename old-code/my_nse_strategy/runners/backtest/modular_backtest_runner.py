...
def validate_arguments(args):
    ...
    if args.start_time:
        try:
            ...
            raise ValueError(f"Invalid start_time format: {args.start_time}. Use ISO format (e.g., '2025-06-18T15:00:00')")
    
    if args.end_time:
        try:
            datetime.datetime.fromisoformat(args.end_time.replace('Z', '+00:00'))
        except ValueError:
            raise ValueError(f"Invalid end_time format: {args.end_time}. Use ISO format (e.g., '2025-06-20T10:00:00')")

def run_single_instrument_backtest(orchestrator: BacktestOrchestrator, 
                                 instrument_id: str,
    ...
    if log_file is None:
        ...
        log_file = str(log_dir / f"{timestamp}-backtest-{instrument_id_str.replace('/', '_')}.log")
    if verbose:
        print(f"\n{'='*60}")
...
def run_all_instruments_backtest(orchestrator: BacktestOrchestrator,
                               start_time: Optional[str],
                               end_time: Optional[str],
                               verbose: bool,
                               max_workers: int):
