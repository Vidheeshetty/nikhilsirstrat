...
class MyNSEStrategy(Strategy):
    ...
    config_class = (
        MyNSEStrategyConfig  # Reference to configuration class from config module
    )
    ...
    def __init__(self, config: MyNSEStrategyConfig):
        ...
        self.breakeven_triggered = (
            False  # Flag indicating if breakeven stop-loss has been triggered
        )
        ...
        # Technical Analysis
        self.rolling_last = deque(
            maxlen=self.config.lookback_intervals
        ...
        try:
            ...
            from strategies.my_nse_strategy.runners.backtest.data_manager import (
                DataManager,
            )

            catalog_path = getattr(config, "catalog_path", None)
            if catalog_path is not None:
                self.instrument = DataManager(catalog_path).get_instrument(
                    str(config.instrument_id)
                )
            else:
                self.log.warning(
                    "No catalog_path in config; expiration check will be skipped."
                )
    ...
    def _load_metadata(self):
        ...
        if not os.path.exists(path) or not os.path.isdir(
            ...
            self.log.error(
                f"Metadata catalog path does not exist or is not a directory: {path}"
            )
        ...
        self.log.info(
            f"Loading metadata from {len(meta_files)} file(s)..."
        )  # Log loading progress
        self.meta_df = pd.concat(
            (pd.read_parquet(f) for f in meta_files), ignore_index=True
        )  # Load all parquet files

        # Set up multi-index for fast lookups
        if (
            not self.meta_df.empty
            and "instrument_id" in self.meta_df.columns
            and "timestamp" in self.meta_df.columns
        ):  # Check required columns exist
            self.meta_df.set_index(
                ["instrument_id", "timestamp"], inplace=True
            )  # Set multi-index for fast lookups
            self.meta_df.sort_index(inplace=True)  # Sort by index for efficient access
            self.log.info(
                f"Loaded {len(self.meta_df)} metadata records."
            )  # Log successful load
        else:
            self.log.error(
                "Metadata DataFrame is missing 'instrument_id' or 'timestamp' columns."
            )  # Log error if columns missing
    ...
    def _validate_quote_tick(self, tick: QuoteTick) -> tuple[bool, float, float]:
        ...
        if bid_price <= 0 or ask_price <= 0:
            self.log.debug(
                f"Invalid prices detected - bid: {bid_price}, ask: {ask_price}"
            )
        ...
        if bid_price > 0:  # Avoid division by zero
            ...
            if spread_pct > 50:  # Skip if spread is more than 50%
                self.log.debug(
                    f"Unreasonable spread detected - bid: {bid_price}, ask: {ask_price}, spread: {spread_pct:.2f}%"
                )
                return False, bid_price, ask_price
        ...
        if bid_price > 10000 or ask_price > 10000:
            self.log.debug(
                f"Extremely high prices detected - bid: {bid_price}, ask: {ask_price}"
            )
            return False, bid_price, ask_price
    ...
    def on_start(self):
        ...
        self.log.info(
            f"Strategy started for instrument: {self.config.instrument_id}"
        )  # Log instrument subscription
        self.log.info(
            "=== STRATEGY ON_START COMPLETED ==="
        )  # Log strategy start completion
    ...
    def on_quote_tick(self, tick: QuoteTick):
        ...
        if self.instrument is not None:
            ...
            if expiration_ns is not None and tick.ts_event >= expiration_ns:
                self.log.info(
                    f"Skipping expired contract: {self.config.instrument_id} (expired at {expiration_ns}, tick {tick.ts_event})"
                )
                return
        ...
        if not is_valid:
            self.log.info(
                f"Skipping tick with invalid prices - bid: {bid_price}, ask: {ask_price}"
            )
            return
        ...
        try:
            meta = self.meta_df.loc[
                (str(tick.instrument_id), tick.ts_event)
            ]  # Get metadata for this tick
        ...
        if (
            len(self.rolling_last) < self.config.lookback_intervals
        ):  # Check if enough data for analysis
            self.rolling_last.append(price)  # Add current price to rolling window
            self.log.info(
                f"Warming up - rolling window size: {len(self.rolling_last)}/{self.config.lookback_intervals}"
            )  # DIAGNOSTIC
        ...
        entry_trigger = price > prev_top * (
            1 + self.config.entry_buffer_pct / 100
        ...
        if self.in_trade:  # Check if already in position
            self.log.info(
                f"Already in trade, skipping entry logic. Position: {self.position}"
            )  # Log skip reason
        else:
            # Apply market filters
            buy_pressure = meta.get("totalBuyQuantity", 0) > meta.get(
                "totalSellQuantity", 0
            ...
            self.log.info(
                f"DIAGNOSTIC - Entry conditions: price={price:.2f}, prev_top={prev_top:.2f}, entry_trigger={entry_trigger}, iv_f={iv_f:.4f}, iv_ok={iv_ok}, oi_change_f={oi_change_f}, oi_ok={oi_ok}"
            )
            self.log.info(
                f"DIAGNOSTIC - Config values: min_iv={self.config.min_iv}, min_oi_change={self.config.min_oi_change}, entry_buffer_pct={self.config.entry_buffer_pct}"
            )

            # ENTRY LOGIC
            if entry_trigger and iv_ok and oi_ok:  # Check all entry conditions
                self.log.info(
                    f"*** ENTRY TRIGGERED! *** Price: {price:.2f}, IV: {iv_f:.4f}, OI: {oi_change_f}"
                )  # Log entry trigger
                ...
                try:
                    ...
                    if self.instrument is not None:
                        ...
                        if expiration_ns is not None and tick.ts_event >= expiration_ns:
                            self.log.info(
                                f"Skipping entry order for expired contract: {self.config.instrument_id} (expired at {expiration_ns}, tick {tick.ts_event})"
                            )
                            return
                    # Submit the order
                    ...
                    self.tp_price = price * (
                        1 + self.config.tp_pct / 100
                    )  # Take-profit at percentage
                    ...
                    self.log.info(
                        f"Trade parameters set - Entry: {price:.2f}, SL: {self.sl_price:.2f}, TP: {self.tp_price:.2f}"
                    )  # Log trade setup

                except Exception as e:
                    self.log.error(
                        f"ERROR creating/submitting order: {e}"
                    )  # Log order error
                ...
                if not entry_trigger:
                    self.log.info(
                        f"Entry not triggered: price {price:.2f} <= prev_top {prev_top:.2f} * (1 + {self.config.entry_buffer_pct / 100:.4f}) = {prev_top * (1 + self.config.entry_buffer_pct / 100):.2f}"
                    )
                if not iv_ok:
                    self.log.info(
                        f"IV filter failed: {iv_f:.4f} < {self.config.min_iv}"
                    )
                if not oi_ok:
                    self.log.info(
                        f"OI filter failed: {oi_change_f} < {self.config.min_oi_change}"
                    )
        ...
        last_n_minutes = 30
        last_n_start = (
            datetime.combine(datetime.today(), session_end_time)
            - timedelta(minutes=last_n_minutes)
        ).time()
        if tick_time >= last_n_start:
            ...
            if self.in_trade and self.direction == "long":  # Check if in long position
                ...
                if price <= self.sl_price:  # Check if stop-loss hit
                    self.log.info(
                        f"*** STOP LOSS HIT! *** Price: {price:.2f}, SL: {self.sl_price:.2f}"
                    )  # Log stop-loss
                ...
                # Take-profit check
                    ...
                    self.log.info(
                        f"*** TAKE PROFIT HIT! *** Price: {price:.2f}, TP: {self.tp_price:.2f}"
                    )  # Log take-profit
                    self._exit_position("TP", price, tick.ts_event)  # Exit position
                    return  # Exit early
                # Breakeven trigger check
                elif not self.breakeven_triggered and price >= self.entry_price * (
                    1 + self.config.breakeven_trigger_pct / 100
                    ...
                    self.log.info(
                        f"Breakeven triggered - SL moved to entry price: {self.entry_price:.2f}"
                    )  # Log breakeven
                # Trailing stop-loss update
                elif (
                    prev_bottom > self.sl_price
                ):  # Check if new low is higher than current stop-loss
                    old_sl = self.sl_price  # Store old stop-loss
                    self.sl_price = prev_bottom  # Update to new low
                    self.log.info(
                        f"Trailing stop updated - SL: {old_sl:.2f} -> {self.sl_price:.2f}"
        ...
        try:
            ...
            if self.instrument is not None:
                ...
                if expiration_ns is not None and timestamp >= expiration_ns:
                    self.log.info(
                        f"Skipping exit order for expired contract: {self.config.instrument_id} (expired at {expiration_ns}, tick {timestamp})"
                    )
                    return
            ...
            if self.current_trade:  # Check if trade is being tracked
                self.current_trade.update(
                    {  # Update trade details
                        ...
                        "pnl": (price - self.entry_price) * self.config.position_size
                        if self.direction == "long"
                        else (self.entry_price - price)
                        * self.config.position_size,  # Calculate P&L
                    }
                )
                ...
                self.log.info(
                    f"Trade completed: {self.current_trade}"
                )  # Log trade completion

        except Exception as e:
            self.log.error(
                f"ERROR creating/submitting exit order: {e}"
    ...
    def on_event(self, event: Event):
        ...
        # Handle order filled events
            ...
            if (
                event.client_order_id in self.active_orders
            ):  # Check if order in active orders
    ...
    def _reset_position(self):
        ...
        self.log.info(
            "Position state reset - ready for next trade"
        )  # Log position reset
    ...
    def on_stop(self):
        ...
        self.log.info(
            f"Strategy completed. Total trades: {len(self.trades)}"
        )  # Log total trades

        # Log final statistics
        if self.trades:  # Check if any trades completed
            total_pnl = sum(
                trade.get("pnl", 0) for trade in self.trades
            )  # Calculate total P&L
            self.log.info(f"Total P&L: {total_pnl:.2f}")  # Log total P&L

        self.log.info(
            "=== STRATEGY ON_STOP COMPLETED ==="
        )  # Log strategy stop completion
